...
"""
    bfut_NfsTgaConverter (HSto3).py - convert NFSHS tga texture alpha channel to NFS3

USAGE
    python "bfut_NfsTgaConverter (HSto3).py" </path/to/file.tga> [/path/to/output.tga]
    python "bfut_NfsTgaConverter (HSto3).py" [</path/to/directory>]
...
CONFIG = {
    "conversion" : "43",  # "34"|"43"|"4M"|"M4"|"3M"|"M3"
...
def main():
    ...
    def shift_colors(in_path, out_path, conversion):
        ...
        tga_header2 = buf[offset:offset + 0x2]
        ...
        # Select pixels
        idx_x00 = np.argwhere(alpha <= 0x00 + 0x6)  # 3,4,M  # only used for stats
        idx_xFF = np.argwhere(alpha >= 0xFF - 0x6)  # 3,4,M  # only used for stats
